...
export const extractCommandMetadata = (command: Node): CmdSuccess | CmdError => {
    ...
    if(rawMeta !== null) {
        ...
        namespace = detectVoNamespace(rawMeta, command);
        ...
        if(rawMeta.shorthand) {
            const schemaConversion = convertShorthandObjectToJsonSchema(schema, namespace);
...
export const extractEventMetadata = (event: Node): EvtSuccess | EvtError => {
    ...
    if(rawMeta !== null) {
        ...
        namespace = detectVoNamespace(rawMeta, event);
